...
policy1_dt[, gas    := as.character(gas)]
...
claims_amount_dt <- claims1_dt %>%
    ...
    arrange(-total_amount, -num_claim)

policyclaim_dt <- policy1_dt %>%
    left_join(claims_amount_dt, by = 'policy_id') %>%
    mutate(total_amount = replace(total_amount, is.na(total_amount), 0))
```
...
ggplot(claims_amount_dt) +
    geom_line(aes(x = seq_along(total_amount) / length(total_amount)
                 ,y = sort(total_amount))) +
    scale_y_log10(label = dollar) +
    xlab("Cumulative Probability") +
    ylab("Claim Amount")
...
ggplot(policyclaim_dt[total_amount > 0]) +
    geom_boxplot(aes(x = region, y = total_amount)) +
    scale_y_log10(label = dollar) +
...
ggplot(policyclaim_dt[total_amount > 0 & total_amount < 25000]) +
    geom_boxplot(aes(x = region, y = total_amount)) +
#    scale_y_log10(label = dollar) +
    ggtitle("Boxplot of Total Claims on a Policy by region")
```
