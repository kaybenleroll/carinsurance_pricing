...
ggplot(claimdata_dt) +
    ...
    geom_point(aes(x = claim_amount, y = exp(predict(model4)))) +
    expand_limits(y = 0) +
    xlab("Claim Amount") +
...
ggplot(claimdata_dt) +
    geom_line(aes(x = claim_amount, y = claim_amount), size = 0.25) +
    geom_point(aes(x = claim_amount, y = exp(predict(model5)))) +
    expand_limits(y = 0) +
    xlab("Claim Amount") +
    ylab("Predicted Amount")
```
...
ggplot(claimdata_dt) +
    geom_line(aes(x = claim_amount, y = claim_amount), size = 0.25) +
    geom_point(aes(x = claim_amount, y = exp(predict(model6)))) +
    expand_limits(y = 0) +
    xlab("Claim Amount") +
    ylab("Predicted Amount")
```
...
model7 <- lm(log(claim_amount) ~ power + brand + cat_car_age
            ,data = claim_5000_dt)
...
ggplot(claim_5000_dt) +
    geom_line(aes(x = claim_amount, y = claim_amount), size = 0.25) +
    geom_point(aes(x = claim_amount, y = exp(predict(model7)))) +
    expand_limits(y = 0) +
    xlab("Claim Amount") +
    ylab("Predicted Amount")
```
...
model8 <- glm(claim_amount ~ power + brand + cat_car_age
             ,family = Gamma(link = 'log')
             ,data = claim_5000_dt)
...
ggplot(claim_5000_dt) +
    geom_line(aes(x = claim_amount, y = claim_amount), size = 0.25) +
    geom_point(aes(x = claim_amount, y = exp(predict(model8)))) +
    expand_limits(y = 0) +
    xlab("Claim Amount") +
    ylab("Predicted Amount")
```
...
We first partition on `power` on `driver_age` and look at the
distribution of the log of the claim amount.

```{r claim_distrib_power_driverage, echo=TRUE}
ggplot(claim_5000_dt) +
    geom_density(aes(x = log10(claim_amount))) +
    facet_wrap(~ cat_driver_age, scales = 'free') +
