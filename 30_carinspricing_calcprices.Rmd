...
claimdata_dt <- policyclaim_dt %>%
    select(-contains("claim_count"), contains("total_claims")) %>%
...
validation_dt <- train_dt %>%
    sample_n(size = 50000, replace = FALSE)

trainpolicies_dt <- train_dt %>%
    filter(!policy_id %in% validation_dt$policy_id)

glimpse(trainpolicies_dt)
glimpse(validation_dt)
```
...
valid_claimcount <- sum(validation_dt$claim_count)


claim_rate <- predict(claim_model_07, newdata = validation_dt
                     ,type = 'response')

sim_rates <- claim_rate * validation_dt$exposure
...
run_mc_sim <- function() {
    model_coef <- as.vector(coef(arm::sim(claim_model_05, n.sim = 1)))
...
run_mc_sim <- function() {
    model_coef <- as.vector(coef(arm::sim(claim_model_07, n.sim = 1)))
...
data_mat <- t(model.matrix(formula(claim_model_05), validation_dt))
...
run_mc_sim <- function() {
    ...
    sim_rates <- exp(output) * validation_dt$exposure
...
data_mat <- t(model.matrix(formula(claim_model_07), validation_dt))
...
print(c(dollar(total_train_premium)
       ,dollar(total_train_claims)
       ,dollar(total_train_premium - total_train_claims)))
```
...
validquotes_dt <- premium_quoter(validation_dt)

validation_dt <- validation_dt %>%
    cbind(validquotes_dt)

summary(validation_dt$price_quote)
```

```{r pricing_check_validation_plot, echo=TRUE}
ggplot(validation_dt) +
    geom_histogram(aes(x = price_quote), binwidth = 10) +
    ggtitle("Histogram of the Calculated Price Quotes for the Validation Dataset")

total_valid_premium <- sum(validation_dt$price_quote)
total_valid_claims  <- sum(validclaim_dt$claim_amount)

print(c(dollar(total_valid_premium)
       ,dollar(total_valid_claims)
       ,dollar(total_valid_premium - total_valid_claims)))
```
...
write.csv(validation_dt,    file = 'data/valid_data.csv', row.names = FALSE)
...
write_feather(validation_dt,    path = 'data/valid_data.feather')
